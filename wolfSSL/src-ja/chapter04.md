...
wolfSSLは**SSL 3.0**、**TLS**（**1.0**、**1.1**、**1.2、1.3**）、および**DTLS**（**1.0**、**1.2**、**1.3**）をサポートしています。
...
同様に、サーバー側で[`wolfSSLv23_server_method()`](group__Setup.md#function-wolfsslv23_server_method)関数を使用することで、TLS 1.0 〜 TLS 1.3のプロトコルバージョンをサポートするクライアントを処理できます。
...
ML-KEMとML-DSAはどちらも、暗号学的に重要な（実用規模の）量子コンピュータに耐性を持つように設計された公開鍵アルゴリズムです。
